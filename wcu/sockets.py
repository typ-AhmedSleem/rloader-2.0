...
def get_my_addr():
    return sockets.gethostbyaddr(sockets.gethostname())[0]
...
class ServerSocket:
    ...
    def __init__(self) -> None:
        ...
        self._server = sockets.socket(sockets.AF_INET, sockets.SOCK_STREAM)
    ...
    def bind(self, address):
        self._server.bind(address.to_tuple() if type(address) is NetAddress else address)
        self.__source_address = address if type(address) is NetAddress else NetAddress(address[0], address[1])  # type: ignore
...
class ClientSocket:
    ...
    def __init__(self, socket: sockets.socket | None = None) -> None:
        # Socket
        if socket:
            self._socket = socket
        else:
    ...
    def makefile(self, mode):
        return self._socket.makefile(mode)
